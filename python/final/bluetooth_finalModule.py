...
async def scan_and_connect():
    while True:
        ...
        if target_device is None:
            ...
            service_uuid, char_uuid = await discover_uuids(target_device.address)
            if service_uuid and char_uuid:
                await handle_connection(target_device.address, service_uuid, char_uuid)
            else:
                print(f"Failed to discover required UUIDs. Retrying in {RECONNECT_DELAY} seconds...")
                await asyncio.sleep(RECONNECT_DELAY)
...
async def discover_uuids(address):
    try:
        ...
        print(f"Failed to discover UUIDs: {e}")
        return None, None
