...
void draw() {
  ...
  drawGraph(yawValues, color(255, 255, 0), initialPositionGraph+(spacing*2));
  drawGraph(pitchAngularSpeedValues, color(255, 0, 255), initialPositionGraph+(spacing*3));
  drawGraph(rollAngularSpeedValues, color(0, 255, 255), initialPositionGraph+(spacing*4));
  drawGraph(yawAngularSpeedValues, color(255, 165, 0), initialPositionGraph+(spacing*5));
  ...
  fill(255, 255, 0);
  text("Yaw: " + currYaw, 0, initialPosition + (spacing*2));
  fill(255, 0, 255);
  text("Pitch Angular Speed: " + pitchAngularSpeed, 0, initialPosition + (spacing*3));
  fill(0, 255, 255);
  text("Roll Angular Speed: " + rollAngularSpeed, 0, initialPosition + (spacing*4));
  fill(255, 165, 0);
  text("Yaw Angular Speed: " + yawAngularSpeed, 0, initialPosition + (spacing*5));
}
...
float[] parseStringData(String data) {
  ...
  if (data.length() > 0 && data.charAt(0) == '<' && data.charAt(data.length() - 1) == '>') {
    ...
    // Check if there are exactly six elements
    if (parts.length == 6) {
